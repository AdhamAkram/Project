...
$sql = "SELECT event_id, artist_name FROM events;";
...
<button class="button button-black width-auto  book-ticket-btn" style="margin : 1px; margin-top: 1%;"  id="addNewmatchButton" type="submit"  name="add">Add new event</button></div>
        ...
        <?php
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<option value='$event_id' " . ($selectedOption == "$event_id" ? 'selected' : '') . ">$event_id - $name</option>";
            }
            ?>
        </select>
    </form>
  ...
  $sqlData = "SELECT * FROM $table_name WHERE event_id LIKE '%$selectedOption%'";
  ...
  if ($resultUpdate) {
    ...
    elseif (isset($_POST['delete']) ) {
        ...
        $eventIdToDelete= $_POST['event_id'];
          ...
          $sqlDelete = "DELETE FROM $table_name WHERE event_id = '$eventIdToDelete'";
     ...
     if (isset($_POST['add'])) {
      echo '<script>window.location.href = "admin-add-events.php";</script>';
  } 
    }
